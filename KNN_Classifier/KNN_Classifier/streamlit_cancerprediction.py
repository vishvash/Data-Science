...
def predict(data, user, pw, db):
    ...
    predictions = pd.DataFrame(model.predict(np.array(newprocessed2)), columns = ['diagnosis'])
...
def main():  
    ...
    st.sidebar.title("Caneer Prediction: DB credentials")

    # st.radio('Type of Cab you want to Book', options=['Mini', 'Sedan', 'XL', 'Premium', 'Rental'])
    html_temp = """
    <div style="background-color:tomato;padding:10px">
    <h2 style="color:white;text-align:center;">Cancer Prediction Model</h2>
    </div>
