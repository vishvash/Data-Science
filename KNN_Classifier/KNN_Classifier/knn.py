...
Business Problem:
There are a lot of assumptions in the diagnosis pertaining to cancer. 
In a few cases radiologists, pathologists and oncologists go wrong in 
diagnosing whether tumor is benign (non-cancerous) or malignant (cancerous). 
Hence team of physicians want us to build an AI application which will predict 
with confidence the presence of cancer in a patient. This will serve as a 
...
cancerdata = pd.read_csv(r"C:/Users/Lenovo/Downloads/Study material/Data Science/KNN_Classifier/KNN_Classifier/cancerdata.csv")
...
cancerdf = pd.read_sql_query(text(sql), con = engine.connect())
