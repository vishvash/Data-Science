...
engine = create_engine("mssql://@{server}/{database}?driver={driver}"
                             .format(server = "360DIGITMG\SQLEXPRESS",        # server name
                                   database = "loanSales_Category",                  # database
                                   driver = "ODBC Driver 17 for SQL Server")) # driver name
'''

# Load the data into Python dataframe for bulk load to SQL
sales = pd.read_csv(r"C:/Users/Lenovo/Downloads/Study material/Data Science/DT Model with Flask/Assignment/Decision Tree/ClothCompany_Data.csv")

# Load the dataframe into database 
sales.to_sql('sales', con = engine, if_exists = 'replace', chunksize = 1000, index = False )
###########
...
# Convert numerical "Sales" data to categorical using cut function
data['Sales_Category'] = pd.cut(data['Sales'], bins=sales_bins, labels=sales_labels, right=False)
...
data['Sales_Category'].value_counts()


# Data split into Input and Output
X = data.iloc[:, :10] # Predictors 

y = data['Sales_Category'] # Target 
...
dot_data = tree.export_graphviz(DT_best, filled = True, 
                                ...
                                class_names = ['Low_Sales', 'Medium_Sales', 'High_Sales'],
                                out_file = None)
...
def cross_validation(model, _X, _y, _cv=5):
    ...
    Parameters
    ----------
    model: Python Class, Sales_Category=None
          This is the machine learning algorithm to be used for training.
    _X: array
          This is the matrix of features.
    _y: array
          This is the target variable.
    _cv: int, Sales_Category=5
