...
uni = pd.read_excel(r"C:\Users\Lenovo\Downloads\Study material\Data Science\Hierarchical Clustering_Hands-on\Hierarchical Clustering_Hands-on\University_Clustering.xlsx")
...
df_norm = df_clean.drop(columns = "State")
cols_n = list(df_norm.columns)


# Train the data preprocessing pipeline on data
df_pipelined = pd.DataFrame(pipe1.fit_transform(df_norm), columns = cols_n, index = df_clean.index)

df_pipelined = pd.concat([df_clean.State, df_pipelined], axis = 1) 
df_pipelined.head()
